"""Generate Math Module 2 stem figures for the 2025-march-int-c mock.

Three items carry artwork: Q10 (line on the xy-plane), Q12 (shaded half-plane)
and Q16 (decaying exponential). Geometry was measured off the page screenshots
by pixel-tracing the axes and the plotted curve, so the SVGs reproduce the
original extents, tick labels and marked points.

Run:  py -3 scripts/_gen_math2_figs_marchintc.py
"""
...
def q10():
    ...
    p.dot(0, 9)
    p.dot(-2, -3)
    p.save("math2-q10-line-graph.svg")


def q12():
    """Half-plane x + 7y >= -77: shading above the line y = -x/7 - 11."""
    ...
    p.save("math2-q12-shaded-half-plane.svg")


def q16():
    """Graph of y = f(x) + 2 = -3^x + 5, clipped at y = -5."""
    ...
    p.origin()
    p.save("math2-q16-exponential-graph.svg")
