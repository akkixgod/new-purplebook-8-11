...
const DATA_DIR = path.join(__dirname, "data", "2026-march-int-a");
  ...
  for (const modNum of ["1", "2"] as const) {
    ...
    for (let i = 0; i < meta.answers.length; i++) {
      ...
      if (imageUrl && (imageUrl.includes("/pages/") || imageUrl.startsWith("NEEDS_SVG"))) {
        throw new Error(
          `Bad imageUrl for ${section} M${modNum} Q${order}: ${imageUrl}`
        );
      }
...
async function main() {
  ...
  console.log("Seeding full 2026 March Int-A (exact text + figures)...");
