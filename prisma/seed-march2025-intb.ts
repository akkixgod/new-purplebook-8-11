...
const DATA_DIR = path.join(__dirname, "data", "2025-march-int-b");
...
async function main() {
  ...
  console.log("Seeding 2025 March Int-B (ENGLISH + MATH)...");
  ...
  // Remove empty placeholder tests for this date/version if any
  const empties = await prisma.test.findMany({
    where: { year: 2025, month: 3, version: meta.version },
  ...
  console.log("Card: 2025 → 2025 March Digital → International Form B");
}
